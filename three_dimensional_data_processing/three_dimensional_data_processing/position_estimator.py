...
class ThreeDimensionalPositionEstimator(Node):
    ...
    def estimate_callback(self, request, response):
        ...
        theta = 90-(38.46-self.head_angle)
        head_height = 13.67*math.sin(theta*math.pi/180)
        realsense_height = self.neck_height + (head_height/100)
        
        response.point.x = centroid_x / 1000
        response.point.y = centroid_y / 1000
        response.point.z = centroid_z / 1000 + realsense_height
        self.get_logger().info("x:%f, y:%f, z:%f", response.point.x, response.point.y, response.point.z)
